...
-- Group all the individual clicks up to the session level to calculate metrics.
session_metrics AS (
    SELECT 
        session_id,
        user_id,
        ...
        -- Use conditional aggregation (SUM CASE) to count how many times they did specific actions.
    ...
    -- Group by the descriptive columns so the aggregate functions work correctly on the rest.
    GROUP BY 
        session_id, 
        user_id, 
        device
